...
const imagensFixas = {
  ...
  "Antonia": "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQ9BdzZ0wDIMjRKqEApPex0PGkm6wuvag8K7XA8l4YdKoFpwTWYWQusvTxWE1Zmqo9c5cU&usqp=CAU",
  "Rafaelle": "https://encrypted-tbn2.gstatic.com/images?q=tbn:ANd9GcTqHIoaUJhAmGtTMYoTBmFDchbW_oZF2jyRZtwHQygjvlWg-P-g96eZjRjRfsgD3cQM7XQozA-4NthJGm0fe4VrJIOLE0E4iKyiaXpYMA",
  ...
  "Angelina": "https://www.cob.org.br/_next/image?url=https%3A%2F%2Fadmin.cob.org.br%2Fuploads%2FAngelina_prata_com_a_selecao_nos_Jogos_Paris_2024_ec81215a91.jpeg&w=3840&q=75",
...
export default function Escalacao() {
  ...
  useEffect(() => {
    ...
    if (savedEscudo) setEscudo(savedEscudo);
  }, []);
    ...
    <div className="min-h-screen bg-pink-100 flex flex-col items-center p-4">
      ...
      <div className="flex items-center justify-between w-full max-w-md mb-4">
        <div className="flex items-center gap-2">
          {escudo ? (
            <img src={escudo} alt="escudo" className="w-12 h-12 rounded-full border-2 border-gray-400" />
          ) : (
            <img src="https://via.placeholder.com/40" alt="escudo" className="rounded-full w-10 h-10" />
          )}
      ...
      {/* CAMPO */}
      <div id="campo" className="relative w-full max-w-md aspect-[2/3] bg-green-600 rounded-lg overflow-hidden border-4 border-white">
        ...
        {formacoes[formacao].map((pos, index) => {
          ...
          return (
            <div key={index} className="absolute -translate-x-1/2 -translate-y-1/2 flex flex-col items-center" style={{ left: pos.x, top: pos.y }}>
              {selecionando === index ? (
                <div className="bg-pink-200 rounded-xl shadow-lg p-2 border border-pink-400 w-48">
                  <div className="text-xs text-gray-600 mb-2 px-1">Escolha jogadora ({pos.pos})</div>
                  <div className="max-h-48 overflow-auto">
                    {(jogadorasPorPosicao[pos.pos] || []).map((j) => (
                      <button key={j} onClick={() => selecionarJogadora(index, j)} className="flex items-center gap-3 w-full text-left p-2 rounded-lg hover:bg-pink-300 transition">
                        <div className="w-8 h-8 rounded-full overflow-hidden bg-white flex items-center justify-center border">
                          {imagens[j] === "loading" ? <div className="loader"></div> : <img src={imagens[j] || imagensFixas[j] || FALLBACK} alt={j} className="object-cover w-full h-full"/>}
                        </div>
                        <span className="text-gray-800">{j}</span>
                      </button>
                    ))}
                  </div>
                  <button onClick={() => setSelecionando(null)} className="text-xs mt-2 text-center block w-full text-pink-600 hover:text-pink-800 font-semibold">Cancelar</button>
                </div>
              ) : (
                <button
                  onClick={() => setSelecionando(index)}
                  className="bg-white rounded-full border-2 border-pink-400 w-16 h-16 flex items-center justify-center overflow-hidden"
                  title={`Posição: ${pos.pos}\nClique para escolher jogadora`}
                >
                  {imagemDaJogadora === "loading" ? <div className="loader"></div> : imagemDaJogadora ? (
              ...
              <span className="text-xs mt-1 text-white font-semibold drop-shadow-lg max-w-16 truncate">{nomeSelecionado || pos.pos}</span>
            </div>
          );
        })}
      </div>
      ...
      <div className="flex gap-4 mt-6">
        <button onClick={exportarImagem} className="bg-pink-400 px-4 py-2 rounded text-white font-semibold hover:bg-pink-500 transition">Exportar Escalação</button>
        <Link to="/home" className="bg-gray-300 px-4 py-2 rounded text-gray-800 font-semibold hover:bg-gray-400 transition">Voltar</Link>
      </div>
    </div>
  );
}
