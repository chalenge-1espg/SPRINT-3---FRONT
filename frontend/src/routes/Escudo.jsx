...
const TelaEscolhaEscudo = () => {
    ...
    const renderSelectedEscudo = () => {
        ...
        switch (escudoSelecionado) {
            ...
            case 'triangulo':
                shapeClasses = "";
                containerStyle = { clipPath: 'polygon(50% 0%, 0% 100%, 100% 100%)' };
                break;
            default:
                shapeClasses = "rounded-full";
                break;
    ...
    return (
        <div className="min-h-screen bg-gradient-to-b from-gray-800 to-green-900 text-white p-4 flex flex-col items-center justify-center">
            ...
            <div className="flex justify-center space-x-4 mb-8">
                <div 
                    className={`w-20 h-20 bg-gray-600 rounded-lg cursor-pointer ${escudoSelecionado === 'quadrado' ? 'border-4 border-yellow-400' : ''}`}
                    onClick={() => handleEscudoClick('quadrado')}
                ></div>
                <div 
                    className={`w-20 h-20 bg-gray-600 rounded-full cursor-pointer ${escudoSelecionado === 'circulo' ? 'border-4 border-yellow-400' : ''}`}
                    onClick={() => handleEscudoClick('circulo')}
                ></div>
                <div 
                    className={`w-20 h-20 bg-gray-600 cursor-pointer ${escudoSelecionado === 'triangulo' ? 'border-4 border-yellow-400' : ''}`}
                    style={{ clipPath: 'polygon(50% 0%, 0% 100%, 100% 100%)' }}
                    onClick={() => handleEscudoClick('triangulo')}
                ></div>
            </div>

            <h2 className="text-lg font-bold mb-4">MANDE A SUA IMAGEM PARA O CLUBE</h2>
